--[[
	/ Emperean Reanimate V2
	/ made by: emperss#0
	/ join: discord.gg/5PMtk6PJf5
	/ dont steal or change credits
	"what the hell is an optimization?"
	
	petition to erase future.destiny from this world pls
	(only vote is yes)
	https://www.youtube.com/watch?v=I9PGNZb32uY
]]
		...
		if HumanoidRootPart then
			for Index, Table in pairs(Targets) do
				...
				while Target and HumanoidRootPart do
					...
					if LinearVelocity.Magnitude >= 100 then
						break
					end
...
local function Fling(Target)
	...
	if typeof(Target) == "Instance" then
		...
		if IsA(Target, "BasePart") then
			Adornee = FindFirstAncestorOfClass(Target, "Model") or Target
			...
			tableinsert(Targets, { Target = Target, Highlight = Highlight })
		end
	end
end
